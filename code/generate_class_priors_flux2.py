...
def validate_config(cfg: DictConfig):
    ...
    if cfg.prior_generation.resolution % 16 != 0:
        raise ValueError("prior_generation.resolution must be divisible by 16 (FLUX2 vae_scale_factor * 2)")
    if cfg.prior_generation.weight_dtype not in WEIGHT_DTYPES:
...
@hydra.main(version_base=None, config_path="conf/flux2", config_name="generate_class_priors")
def main(cfg: DictConfig):
    ...
    device = get_device()
    weight_dtype = WEIGHT_DTYPES[cfg.prior_generation.weight_dtype]
    ...
    res = cfg.prior_generation.resolution
    generator = torch.Generator(device).manual_seed(cfg.runtime.seed)
    ...
    for i in tqdm(range(0, num_to_generate, batch_size), desc="Generating class priors"):
        ...
        images = pipeline(
            prompt=[cfg.prior_generation.class_prompt] * current_batch,
            height=res,
            width=res,
